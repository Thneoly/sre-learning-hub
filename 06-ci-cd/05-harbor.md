...
> 模块：06-ci-cd ｜ 建议时长：4 小时 ｜ 关联认证：CKS-供应链安全（辅助：trivy/cosign 落到仓库侧）/ —（无直接考点，国内企业 registry 事实标准）
...
第 2、3、5 三条对 CKS 视角尤其致命——供应链安全的四段防线（../09-cks/04-supply-chain-security.md 第 1 节的"仓库段"）在裸 registry 上是空的。
...
Trivy 的 CLI 用法（severity 过滤、`--exit-code` 门禁）第 09-cks/04 章已系统讲过，这里只讲仓库侧集成：
...
09-cks/04 第 4 节讲过 cosign 的机制：签名作为附带制品存进仓库（`<repo>:sha256-<digest>.sig`），验证方持公钥校验。Harbor 与它的关系分三层：
...
3. **准入层（真正挡人的地方不在 Harbor）**：签名验证必须发生在**部署时**——sigstore policy-controller、Kyverno 或 OPA Gatekeeper 做集群准入（09-cks/04 第 5 节的 admission 链路）。Harbor 的角色是"签名与公钥的管理面 + 展示面"
...
流水线编排顺序（CI 内三步，缺一不可，串起 09-cks/04 与本模块 labs/06-supply-chain-gates 的双闸门设计）：

```
CI package job:
  docker build → trivy 扫描(--exit-code 1，漏洞闸门)
               → docker push
               → cosign sign --key <CI 里的私钥>(09-cks/04 第 4 节，签名闸门)
集群侧:
  Kyverno/policy-controller 验签失败 → Pod 创建被拒(09-cks/04 第 5 节)
```
...
- cosign（与 09-cks/04 第 4 节互参）：https://docs.sigstore.dev/
