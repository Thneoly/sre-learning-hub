...
> 模块：10-pca ｜ 建议时长：每题 5~15 分钟，全部完成约 12 小时（可分 6 次刷完） ｜ 关联认证：PCA-PromQL（核心） / CKS-监控（部分）
>
> 本文按 `_meta/STYLE.md` 的"题库文件模板"（Lab 三件套的豁免形态，仅限 10-pca）组织：题目正确性依赖 Prometheus UI 查询结果，逐题到"如何验证你的答案"一节给出的环境中自查。
