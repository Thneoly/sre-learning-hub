...
阶段 P  性能+复盘  弹性 1~2 周 21-perf-testing + 22-incident-stories（可穿插，也可主线后集中）
...
| 2 | `01-linux/04-06`（进程/内核网络栈/性能分析）+ lab 02；`07`（用户与权限）与本章并行或紧随 | 能 60 秒内定位 CPU 高/内存涨/磁盘满 |
...
| 9 | `04-k8s/12-14`（`15` 扩展模型选学：CRD/准入链，CKS 与生态认知的底座）+ `05-cka/00-06` + 题库手册 16 题 + labs 全部 | etcd 备份恢复独立完成；题库 <7 分钟/题 |
...
| 22 | `14-data-streaming/kafka`（3 章）+ `flink`（3 章，含 03 运维与状态：rescale/checkpoint 排障/CDC 入湖）+ labs | 解释 ISR/KRaft；定位一次反压 |
...
| 25+（选学，可后置到求职冲刺） | `15-sre/06-07`（发布工程/灾备与业务连续性） | 06 与阶段 3 交付平台呼应；07 的 RTO/RPO 对账衔接 20-lifecycles 与 16-cloud/05 |
...
`16-cloud/01-05`（01-03 必修；`04` 弹性伸缩/`05` 云上容灾与备份，05 与 15-sre/07 的 RTO/RPO 联动）+ VPC 设计 lab；有阿里云账号就实操，没有做纸面设计。可顺手报名阿里云 ACP。

**🔁 闭环 8**：把前面闭环产出的整套系统做一次"上云设计"——VPC/交换机/安全组规划、节点与存储选型、估算一份月账单（按量 vs 包年包月的临界点），输出一页架构图 + 成本测算；有账号就在云上真实拉起最小可用版。

### 阶段 9 · 差异化（周 29）

`17-aiops-llm/01-05` + lab（`05` 私有化 LLM 端点：Ollama/vLLM，04 章 agent 的验收端点）；补 `02-programming/05`（Go）；有余力加 `02-programming/07`（Operator 开发，7 小时大章，K8s 控制循环的亲手实现）。
**🔁 闭环 9**：用 LLM 辅助排障一次注入故障（完整记录对话与验证），修复后把复盘沉淀进知识库——这就是"运维 LLM 化"的个人实证。

### 阶段 P · 性能与复盘（弹性 1~2 周，不计入 29 周主线）

| 弹性周 | 材料 | 里程碑 |
|---|---|---|
| +1 | `21-perf-testing/01-02`（压测工具 / 容量规划）+ 第 1 章实战演练 A~D | 用 k6 压出拐点，写一份完整容量评估报告（外推 + N-1 验证 + 中间件基线表） |
| +2（故事集可拆半周） | `22-incident-stories/01`（四个故事）+ 章末"靶场演练路径"任选两条 | 每条 60 分钟演练配一份 20 行迷你复盘（套 15-sre/04 模板） |

穿插建议（比压在最后更有效）：`21-perf-testing` 可在阶段 5（可观测）结束后插入——读拐点与水位需要 PromQL 与 exporter 的底子；`22-incident-stories` 放在阶段 7（方法论）之后读，复盘模板的体感才完整。求职冲刺期优先把四个故事的"探测与误判"段过两遍——那是面试里"讲一次你排查过的故障"的现成弹药。

**🔁 弹性闭环 P（附加，不计入 10 个主线闭环）**：给自己的业务（或靶场 nginx/Redis）走一次完整容量闭环——压出拐点 → 外推副本数 → N-1 验证 → 落容量评估报告；再任选一个故事在靶场复现并写迷你复盘。两份产出就是简历上"容量管理 + 故障复盘实战"的实证。
