...
| §4 存储与中间件 | 72 | 原 8 条 + PG 8 条 + 16-bigdata 47 条：MySQL/Redis 的主从延迟与复制中断；PG 复制槽 pin WAL、同步名单挂起、Patroni 失多数派全只读；HDFS safemode 卡住 / 丢失块 runbook / 副本挤同机架；YARN 假 OOM 与 RM recovery；Spark OOM 在堆外 / 倾斜三板斧 / FetchFailed；Doris FE 选举阻塞、compaction 版本堆积、label 幂等；CH 副本依赖 ZK/Keeper、Distributed 异步与再平衡；湖仓 commit 冲突、Hudi 积压、孤儿文件 | 日志空洞、quorum、脑旋、再平衡风暴、快照链路五类全部命中 |
