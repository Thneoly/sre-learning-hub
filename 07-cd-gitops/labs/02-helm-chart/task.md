...
> 难度：★★☆ ｜ 考点：chart 开发（values/模板函数）/ release Secret 与 revision / `--wait`、`--atomic`、rollback / OCI registry 分发 ｜ 前置：第 08 章（02-helm）、03-docker/labs/08（本地 registry，需先把它跑起来）、练习集群可用 ｜ 预计 40~60 分钟
