# 01 · Kustomize：无模板的多环境 Manifest 管理

> 模块：07-cd-gitops ｜ 建议时长：4 小时 ｜ 关联认证：CKA-应用管理（考纲 awareness 层的"manifest 管理与模板工具"）/ —（无直接考点，多环境交付的事实标准）
...
Helm 的系统教学见下一章 02-helm.md，这里先给后续章节（中间件、可观测性 labs 里大量 `helm install`）够用的最小集：
