...
`top10.txt` 前 1 行是 hotkey、后 9 行是任意 9 个 word*（它们计数并列，排序取字典序在前者）——check.sh 只断言第一行精确等于 `hotkey 900000`，不约束并列项顺序。`_SUCCESS` 是 Spark 写出器的事务成功标志（与 Hive/HDFS 写路径一致的两阶段提交思路，`18-bigdata/03-hive-warehouse` 会再遇到）。
