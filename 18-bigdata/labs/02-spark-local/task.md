...
> 难度：★★☆ ｜ 考点：18-bigdata/04-spark（shuffle/聚合倾斜/两阶段聚合） ｜ 前置：18-bigdata/labs/01；14-data-streaming/flink 的反压实验（同一现象的批处理版） ｜ 预计 45~60 分钟
...
这和 `14-data-streaming/flink/labs/01` 里 Flink 的热点 key 反压实验是**同一个物理规律的两个投影**：数据按 key 哈希后倾斜到同一个任务，批处理里表现为 straggler task，流处理里表现为反压。YARN/K8s 上部署 Spark 的部分见 `18-bigdata/02-yarn` 与 `04-spark` 章，本 lab 专注计算语义本身。
...
```bash
# [任意节点]
cd 18-bigdata/labs/02-spark-local
chmod +x check.sh
./check.sh
```
