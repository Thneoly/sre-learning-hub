...
> 难度：★★★ ｜ 考点：18-bigdata/07-lakehouse（snapshot/manifest 元数据树 / 主键表 upsert / Flink 入湖链路） ｜ 前置：18-bigdata/00 §2 三巨头速览表、05 §7 湖查询；03-docker 的容器资源限制（labs/06-resource-limits，compose 里写成 `mem_limit`）；14-data-streaming/flink 的 checkpoint（本 lab 的提交语义直接依赖它） ｜ 预计 60~90 分钟
...
**jar 拉不动 / 环境跑不动时的 SIMULATED 模式**：不部署真集群，改为产出三份交付物——完整 compose 文件、可对真环境直接执行的完整 SQL 脚本、warehouse 目录结构说明文档。判分脚本自动识别 full / simulated 两种模式给分（先例：`09-cks/labs/05-runtimeclass`）。
...
7. （SIMULATED 模式）真环境不可用时，在本 lab 目录（`18-bigdata/labs/04-lakehouse-flink-paimon/`）下产出三份交付物：`docker-compose.yml`（同第 1 步要求）、`lakehouse-pipeline.sql`（第 3/4/6 步全部 SQL 合一的完整可执行脚本）、`warehouse-structure.md`（目录树逐项注释文档：snapshot/manifest/bucket 各管什么、一次 checkpoint 如何产生新 snapshot、更新一条主键在树上落成哪些新文件）。
...
```bash
# [任意节点]
cd 18-bigdata/labs/04-lakehouse-flink-paimon
chmod +x check.sh
./check.sh
```
