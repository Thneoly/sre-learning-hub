...
- **`execution.checkpointing.interval: 5s`（本 lab 的命门）**：Paimon sink 的提交动作挂在 checkpoint 上——数据先进 LSM 内存与临时文件，checkpoint 成功后才写 manifest、生成 snapshot，数据对查询可见。不开 checkpoint，写一万行也看不到 snapshot。这与 05 章 §5 的 sink 两阶段提交是同一条原则（`notifyCheckpointComplete` 之后才 commit），也是日后排障的映射：**湖表 snapshot 停止增长 = 上游作业 checkpoint 出问题**（14-data-streaming/flink 的 checkpoint 排障直接搬来用）；
...
真环境不可用时产出三份交付物到本 lab 目录（`18-bigdata/labs/04-lakehouse-flink-paimon/`）：
