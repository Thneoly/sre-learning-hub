...
> 难度：★★☆ ｜ 考点：18-bigdata/01-hdfs（块机制 / NameNode 元数据 / safemode） ｜ 前置：03-docker 容器与资源限制 ｜ 预计 45~60 分钟
...
对应生产语境：块大小影响 map 数与顺序读吞吐（`14-data-streaming` 里 Kafka 的分段文件是同一类"大顺序写"设计），NN 元数据全内存决定集群文件数上限，safemode 是 DN 掉线/块丢失后的第一现场。
...
```bash
# [任意节点]
cd 18-bigdata/labs/01-hdfs-pseudo
chmod +x check.sh
./check.sh
```
