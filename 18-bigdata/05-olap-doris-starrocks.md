...
> 模块：18-bigdata ｜ 建议时长：4 小时 ｜ 关联认证：—（CKA/CKS/PCA 无直接考点；PCA 的 exporter/PromQL 方法直接复用于 FE/BE 监控，Docker 部署复用 03 模块网络知识；本章是 labs/03-doris-quickstart 的理论底座）
...
为什么不能拿交易库直接跑报表（排障视角，见 13-middleware/mysql/03-tuning-troubleshooting.md 的慢查询分型）：
...
为什么也不该把报表塞进 Flink（见 14-data-streaming/flink/01-stream-processing-model.md）：
...
- **副本**默认 3 份落在不同 BE（本质是不同机器），BE 掉线后 FE 的副本调度器自动从健康副本 clone 补齐——这一套和 Kafka 的 ISR 思路一致但粒度不同：Kafka 副本单位是分区，Doris 是 tablet，粒度细得多，所以均衡与修复也快得多（对照 14-data-streaming/kafka/02-replication-and-reliability.md）。
...
FE `http://fe:8030/metrics`、BE `http://be:8040/metrics`，直接接入 08 模块的抓取体系（10-pca/04-instrumentation-exporters.md 的接入方式原样适用）。指标名随版本微调，以端点实际输出为准：
