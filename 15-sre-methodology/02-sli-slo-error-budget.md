...
> 模块：15-sre-methodology ｜ 建议时长：5 小时 ｜ 关联认证：PCA-PromQL / PCA-告警（燃烧率告警直接复用 10-pca 的规则与路由机制）
...
### 3.4 容量型 SLI：饱和度也是服务质量（导学）

第 2.2 节的 SLI 菜单以可用性/延迟/正确性为主——它们回答"**坏没坏**"。还有第四类：**容量型（饱和度）SLI**，回答"**还能坏多久**"。前三类是结果指标，容量型是领先指标：连接池占用 90%、节点 CPU 长期 85%、队列深度持续爬升时，可用性 SLI 还没越线，但离越线只剩一次大促的距离。常用形态：

| 饱和度对象 | 典型口径 | 越线的先兆 |
|-----------|---------|-----------|
| 节点 CPU/内存 | usage / capacity 的水位带（如 <70%） | 调度失败、驱逐前夜（21 章"三本账"） |
| 应用资源 | usage / requests（HPA 内部算的就是它） | HPA 扩容滞后于流量 |
| 连接池/队列 | 占用比、积压深度 | 延迟 SLI 抬升的头部原因 |

窗口式口径的例子（水位处于安全带的时间占比）：

```promql
# [Prometheus Web UI] 容量型 SLI：节点 CPU 水位 < 70% 的时间占比
# < bool 让比较输出 0/1 序列，avg_over_time 的均值就是"达标时间占比"
avg_over_time(
  ((1 - avg by (instance)(rate(node_cpu_seconds_total{mode="idle"}[5m]))) < bool 0.7)[1h:1m]
)
```

它同样能写成 SLO（"水位 < 70% 的时间占比 ≥ 99%"）、同样有预算（1% 的"接近饱和时间"）、同样能接第 5 节的多窗口燃烧率告警——机制与本章完全同构，只是语义从"服务坏了"换成"服务快没余量了"。水位线怎么分绿黄红三档、压测拐点怎么外推成副本数与安全余量、为什么外推会失效（共享瓶颈）、Proactive Scaling 为什么要做在水位抬头之前——这些容量工程方法在 [21-perf-testing/02 · 容量规划](../21-perf-testing/02-capacity-planning.md) 专门展开。先记住结论：**只盯结果型 SLI 的团队是在等事故，容量型 SLI 给你的是提前量**。
...
告警的路由（page 打给谁、ticket 进哪个队列、分组与静默）在 10-pca/05-alerting-alertmanager.md 已展开，本课只管"expr 怎么写、阈值怎么来"。PromQL 基础（rate/聚合/子查询）见 10-pca/03-promql-guide.md。
...
```bash
# [master] 把 Grafana 缩为 0（与 10-pca 告警课同一手法）
...
- 再满 `for: 2m`：变 **Firing**，severity=critical 走向 Alertmanager（路由见 10-pca/05）；
- 同期在 UI 查预算消耗：
