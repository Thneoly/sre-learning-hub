...
注入后时间   ratio_rate1m        ratio_rate5m        告警状态
 ...
 t+45s       ~0.75               ~0.96               inactive（越限条件已满足，等评估链传导）
 t+90s       ~0.75               ~0.93               pending（两窗口均 < 0.9856）
 t+2m30s     ~0.75               ~0.88               firing（for: 1m 满足）
```

阈值核对：可用性比率的越限线是 `1-(1-0.999)*14.4 = 0.9856`，即可用性低于 98.56% 就算越限。按聚合错误率 25% 换算，5m 窗口可用性 = 1 - 0.25×t/300，攒够 1.44% 错误只需约 17 秒——注入后各窗口很快全部越限；t+45s 仍显示 inactive 不是"窗口没泡透"，而是 30s 抓取间隔 + 两级各 30s 的 recording rule 链路把状态传导拖后了（pending 落在 t+1m 前后，firing = pending + for: 1m + 评估对齐）。
