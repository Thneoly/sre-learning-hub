...
features:
  ...
  - icon: 🚀
    title: DevOps 工程域
    details: 优化软件交付流程：流动、反馈、持续学习（三步工作法）。覆盖 06-ci-cd（Git / GitLab CI / Jenkins & GHA / SonarQube / Harbor）· 07-cd-gitops（ArgoCD / Kustomize / Helm / 交付平台）· 08-iac（Ansible / Terraform）。
    link: /06-ci-cd/00-devops-concepts.html
    linkText: 从 DevOps 概念开始
  - icon: 🔭
    title: 可观测与 SRE 域
    details: 优化系统可用性：指标 / 日志 / 追踪三支柱 + SLO、错误预算与混沌工程。覆盖 10-pca · 11-otel · 12-logging · 15-sre-methodology。
    link: /10-pca/00-exam-overview.html
    linkText: 从可观测概念开始
  - icon: 🤖
    title: AIOps 智能域（增值）
    details: 用 LLM 增强判断：辅助排障、RAG 知识库、Agent 护栏。覆盖 17-aiops-llm 与 scripts/faults 故障靶场。
    link: /17-aiops-llm/01-aiops-landscape.html
