...
> 难度：★★★ ｜ 考点：16-bigdata/08（MergeTree 家族 / 主键非索引 / 双表架构 / too many parts） ｜ 前置：16-bigdata/06（ZooKeeper，本 lab 的协调后端）、16-bigdata/08 章（表引擎与 Distributed 的机制都在那章）；03-docker 的 compose 与 `mem_limit` 资源限制（labs/06-resource-limits） ｜ 预计 60~90 分钟（含拉镜像）
