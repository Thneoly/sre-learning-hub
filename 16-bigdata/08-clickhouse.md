...
> 模块：16-bigdata ｜ 建议时长：4 小时 ｜ 关联认证：—（CKA/CKS/PCA 无直接考点；监控接入复用 08-pca 的 exporter/PromQL 体系，docker 部署复用 03 模块；本章是 labs/05-clickhouse-cluster 的理论底座）
...
03 章从 Hive 视角讲过 ORC/Parquet（`16-bigdata/03-hive-warehouse.md` 第 4 节），ClickHouse 把同一思想做进了"实时数据库"：数据按列组织在不可变的 **part** 文件里，而不是按行组织在 B+ 树页里。
...
反过来它就做不了 OLTP（对照 `11-middleware/mysql/01-innodb-fundamentals.md` 第 2 节的 B+ 树）：按主键取一行要把所有列的 part 文件拼回来、点查走的是"定位粒度再扫描"而不是 3 层 B+ 树的 2 次页 IO；数据不可变（immutable part）意味着没有原地更新，改一行等于重写整个列块。**列存用"单行昂贵"换"一批极快"**。
...
- **扩容是手工活**：新增分片后，历史数据不会自动搬迁（Distributed 只影响新写入的路由），要么重灌、要么按 17-distributed/05-sharding-and-rebalancing.md 第 4 节的再平衡窗口方法论手工迁移；可用 `weight` 调新旈权重渐进导流。这是对比 Doris "BE 加入即自动均衡 tablet"最疼的运维差异。
...
| 加了节点数据不均衡 | 新分片只接新写入，历史数据不自动迁移 | 规划期留足分片；迁移按 17-distributed/05 第 4 节窗口方法论手工做，`weight` 渐进导流 |
...
MySQL：聚簇索引按主键组织，根节点常驻内存，2 次页 IO 精确到行（11-middleware/mysql/01 第 2 节）。ClickHouse：主键是稀疏索引，二分只能定位到 8192 行的 granule，然后要解压该 granule 各列的压缩块、扫过最多 8192 行才找到目标——单行点查的解压与扫描成本高几个数量级。反转场景：按主键前缀做大范围扫描并聚合（"某类事件全天的 count/sum"），列存只读 2~3 列 + 压缩块顺序读 + 向量化，B+ 树要把整行读进来且随机 IO，CH 快 1~2 个数量级。本质：行存优化"到一行的路径"，列存优化"过一批列的吞吐"。
</details>
