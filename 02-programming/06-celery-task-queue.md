...
单独的绝对值没有意义，判断公式是消化时间 = LLEN ÷ 完成速率。反例一：吞吐 100 条/s，LLEN=500 只是 5 秒的浪，报警纯属噪音；反例二：吞吐 1 条/s（如每条任务跑 30 分钟、只有几个并发），LLEN=500 意味着数小时甚至数天的积压，用户侧早已超时——这才是 P1。所以告警规则应写成"预计消化时长 > N 分钟"（PromQL 上即 backlog / rate(完成计数)，完成计数来自事件或 worker stats 导出），而不是"LLEN > X"。
</details>
