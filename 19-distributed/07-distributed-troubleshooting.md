...
> 模块：19-distributed ｜ 建议时长：3 小时 ｜ 关联认证：—（无直接考点；本章是 03~06 章的运维出口，也是把全站 SCENARIOS.md 里"分布式类"现象串起来的收口章）
...
- 能说出分布式机制故障在全站 239 条场景索引里的分布位置，并从"现象"快速跳回本章
...
2. **扩容跳过偶数**：3→5 直达，4 不增加容错只增加确认数（[04-k8s/02 章 §3](../04-k8s-fundamentals/02-architecture-and-control-loop.md)、[18-bigdata/06 章 §6.5](../18-bigdata/06-zookeeper.md)）。
...
分区把集群切成两半，各自觉得自己是合法主侧。共识系统里少数派侧**写卡死**（凑不齐 ACK），多数派侧选出新主、term/epoch 跳变——协议层防住了双提交（推导见 [18-bigdata/06 章 §3](../18-bigdata/06-zookeeper.md)）。Redis 是特例：数据面主从没有 quorum，防双主靠外挂哨兵 majority，分区期间旧主还可能吞写（[redis 02 章 §6.4](../13-middleware/redis/02-persistence-and-ha.md)）。
...
## 4. 与 SCENARIOS.md 的衔接：239 条场景里的分布式故障

[SCENARIOS.md](../SCENARIOS.md) 是全站"现象进门"索引（239 条）。分布式机制类故障集中在三个类目：

| 类目 | 条数 | 直接命中分布式机制的 | 对应本章类别 |
|---|---|---|---|
| §1 集群与控制面 | 18 | 5 条：apiserver crash loop 连不上 etcd（【靶场】break-etcd-endpoint）；etcd `alarm NOSPACE` 只读；etcdctl snapshot 报错；+ 11 章的 NOSPACE 处理后仍只读（alarm disarm）、多成员同时 defrag 抖动 | 写路径 ④⑦ 跳；etcd 运维 |
| §4 存储与中间件 | 72 | 原 8 条 + PG 8 条 + 18-bigdata 47 条：MySQL/Redis 的主从延迟与复制中断；PG 复制槽 pin WAL、同步名单挂起、Patroni 失多数派全只读；HDFS safemode 卡住 / 丢失块 runbook / 副本挤同机架；YARN 假 OOM 与 RM recovery；Spark OOM 在堆外 / 倾斜三板斧 / FetchFailed；Doris FE 选举阻塞、compaction 版本堆积、label 幂等；CH 副本依赖 ZK/Keeper、Distributed 异步与再平衡；湖仓 commit 冲突、Hudi 积压、孤儿文件 | 日志空洞、quorum、脑旋、再平衡风暴、快照链路五类全部命中 |
| §9 分布式与共识 | 39 | 本模块与 ZK 章：ZK 4lw 白名单 / 脑旋 / 扩容写不进 / 会话漂移锁丢 / watch 一次性；etcd 失 quorum 拒写 / 频繁切主 / 奇数原则；时钟跳变假尖刺 / 日志倒挂；锁误删 / zombie writer / lease 时钟；脑裂取证 / 失 quorum 先救一台；+ 08~11 章的 Paxos 活锁、CRDT/LWW 丢写与墓碑复活、Consul gossip 不汇合 / DNS TTL 摘除延迟、Nacos gRPC 端口 / Derby 误用 | 五类经典故障 + 共识、一致性、时钟、fencing 全套 |
| §2/§3/§5/§6/§7/§8 | 110 | 间接相关若干（如 webhook fail-closed 属写路径②跳、证书过期是时间敏感故障；Celery 的 acks_late 重复执行是"至少一次"的化身） | 写路径①②跳 |
