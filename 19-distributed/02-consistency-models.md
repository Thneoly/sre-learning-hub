...
> 模块：19-distributed ｜ 建议时长：3.5 小时 ｜ 关联认证：—（无直接考点；本章给出"这个系统承诺读到什么"的判读框架，是参数评审与"读到旧数据"类工单定性的一锤定音依据）
...
四级阶梯在真实系统里不是四个孤岛，而是**同一根轴上的档位**——每家系统按"操作 + 参数"落在某一格，格与格之间用延迟、可用性、元数据明码标价。把阶梯画成谱系，后面所有落位（第 3 节）都是往这根轴上钉钉子：

```
[图] 一致性谱系：越往上承诺越硬、价格越贵（上线自动满足下线的合同）

  ▲ 强   线性一致 ─── 尊重实时序：写已应答，任何客户端从任何副本必见新值
  │           └─ etcd 默认读（ReadIndex 过多数派）/ ZK 写路径
  │      顺序一致 ─── 全局全序存在，但不认实时序
  │           └─ ZK 默认本地读 / Kafka 单分区 FIFO
  │      因果一致 ─── 有因果关系的操作所见序一致，并发写各有所见
  │           └─ MongoDB 因果一致会话（消息捎带会话令牌）
  │      会话保证 ─── 只对"我自己"承诺：读己之写 / 单调读 / 单调写（因果一致的实用子集）
  │           └─ 副本粘性 + 会话令牌——价格最便宜的"够用档"（2.1 节）
  ▼ 弱   最终一致 ─── 只承诺停写后收敛，收敛距离可监控（4.2 节）
              └─ Redis 主从 / DNS / Informer 本地缓存

  每往下一档，就多放开一类"允许读到的旧值"（1.2 节表第三列反向读）；
  每往上一档，都要拿东西买：延迟（quorum 往返）、可用性（分区期间停写）、元数据（因果上下文）。
```

### 1.3 两个必须划清的界限

- **一致性模型 ≠ 隔离级别**：一致性模型讲"读操作能看到哪些写"（对象是**副本间的顺序**）；隔离级别讲"多个事务交错执行时的现象"（对象是**并发事务**）。MySQL 的 RC/RR 靠 MVCC 的 Read View 在**单机**上解决不可重复读（`../13-middleware/mysql/01-innodb-fundamentals.md`，RC 每条语句建视图、RR 首条快照读建视图），与主从复制后的读一致性是两个维度。
...
| 保证 | 内容 | 违反时的业务表现 | 已学出处 |
|---|---|---|---|
| 读己之写（read-your-writes） | 我写完的值，我立刻能读到 | "我改完头像刷新还是旧的" | ZK 要 `sync()` 后本地读才保证（`../18-bigdata/06-zookeeper.md`：本地读可能旧） |
| 单调读（monotonic reads） | 我重复读同一数据，不会越读越旧 | 刷新一次内容"倒回去"了 | Redis 客户端从一个从库切到更落后的从库/新主 |
| 单调写（monotonic writes） | 我的多次写不被乱序观察到 | 消息顺序颠倒 | Kafka 同 key 恒进同一分区就是为此（`../14-data-streaming/kafka/01-log-model-and-architecture.md`） |

### 2.2 脏读 / 不可重复读 / 幻读的分布式化身

| 单机隔离异常 | 分布式化身 | 已学现场的机制 |
|---|---|---|
| 脏读（读到未提交） | 读到**未来会消失的数据**：主库宕机切换后，新主没有这段数据，消息被截断 | Kafka 的 HW 正是为此存在："消费者只能读到 HW 之前的消息"，否则会读到一条随截断消失的消息（`../14-data-streaming/kafka/02-replication-and-reliability.md` 第 2 节）；MongoDB `readConcern: local` 可能读到最终被回滚的写（`../13-middleware/mongodb/02-replicaset-and-sharding.md` 第 4 节）；Flink 下游不开 `isolation.level=read_committed` 会读到未提交事务 |
| 不可重复读（两次读不一致） | **时间倒流**：failover 切到更落后的新主，或客户端换到落后副本，第二次读比第一次还旧 | Redis 哨兵切换后新主落后于旧主的窗口（`../13-middleware/redis/02-persistence-and-ha.md`）；MySQL 从库重放进度不同，两次读打到不同从库 |
...
| ZooKeeper | **写线性一致**；**默认读=顺序一致**（本地内存，可能旧） | ZAB 过半提交保证写全序 | 读要"读己之写"级别就先 `sync()`；leader 挂 2/3 时停写保 C | `../18-bigdata/06-zookeeper.md` 第 3 节 |
| Kafka（单分区） | **顺序一致**（单领导者 + 分区内 FIFO） | 每分区一个 leader，读写都走它；HW 是可见性边界 | `unclean.leader.election` 允许落后副本上位=用丢数据换可用（00 章 CAP 的 A 侧） | `../14-data-streaming/kafka/02-replication-and-reliability.md` |
| Kafka（跨分区/topic） | 无全局序（只有分区内序） | — | 要全局序只能单分区或按 key 分区（牺牲并行度） | 同上 01 章 |
| Redis 主从 | **最终一致** | 异步复制；`WAIT n timeout` 可对单次写等待副本确认 | failover 丢旧主最后一段写入；`min-replicas-to-write` 缩小脑裂窗口 | `../13-middleware/redis/02-persistence-and-ha.md` 6.4 节 |
| MongoDB | 按请求选：`readConcern local`（默认，可能旧/被回滚）→ `majority`（线性基线） | writeConcern `w:majority` 配 readConcern majority 是强一致基线 | 因果一致要 causally consistent session 配合 | `../13-middleware/mongodb/02-replicaset-and-sharding.md` 第 4 节 |
| MySQL 半同步 | 仍是**最终一致**（从库重放异步）；半同步改的是 RPO 不是读语义 | `rpl_semi_sync_master_*`，超时自动降级异步 | 降级后回到纯异步（RPO>0） | `../13-middleware/mysql/02-backup-replication.md` 半同步一节 |
...
| 双实例 Alertmanager | 最终一致（gossip） | gossip 复制 silences/nflog，分区瞬间可能双发通知 | 已知权衡，收敛后去重 | `../10-pca/05-alerting-alertmanager.md` 6.2 节 |

读表三个要点：**强一致都不是白来的**——etcd 每次写要多数派 fsync，ZK 每次写要过半 ACK，这正是两家"不适合当业务存储、只放元数据"的根源；**落位是按操作粒度的**——ZK 同一系统读一套写一套；**降级开关是运维参数评审的实弹靶场**——评审 K8s 业务上 Kafka，就是评审 acks/min.insync.replicas 这几个"一致性/持久性"定价参数（`../14-data-streaming/kafka/02-replication-and-reliability.md` 第 4 节组合矩阵）。

把落位表换个方向读一遍——行=一致性档位，列=三家在练习环境里摸过的系统，每格是"这一档在这家怎么拿到 / 怎么丢掉"。正读反读两遍，"XX 系统什么一致性"的追问就困不住你：

| 一致性档位 | etcd | Kafka | Redis |
|---|---|---|---|
| 线性一致 | **默认读**：ReadIndex 先与多数派确认"要读的成员视图不落后"（演练一实测它的价格） | **不提供**：单分区 leader + HW 只到顺序一致加可见性边界；`acks=all` 买的是持久性，不是读语义 | **仅主库近似成立**：单线程执行序即实时序；failover 瞬间不保（2.2 节"时间倒流"），`WAIT` 也不是线性读（自测 4） |
| 顺序一致 | `--consistency=s` 串行读：读本成员 KV，省掉每读一轮多数派往返 | **单分区 FIFO**：leader 唯一，全序存在；跨分区/跨 topic 无全局序（落位表第 4 行） | 读主库：单主全序成立；读从库即掉出本档 |
| 因果一致 | 不单设档位：默认线性读已覆盖（更强合同自动满足更弱承诺） | 同 key 恒进同分区保住该 key 的因果序；跨 key 跨分区无 | 客户端钉住同一主库期间成立；主从切换即可能违反 |
| 会话保证 | 线性读下免费获得；串行读下"写后立读"不保（2.1 节表） | 单消费者按 offset 前进 = 天然单调读（lag 是收敛距离，不是违约） | 读己之写：写后读都打主库即可；单调读：从库间切换会被打破（2.1 节表） |
| 最终一致 | 串行读打到落后成员时的实际体验；Raft 日志追平即收敛 | 消费者 lag、ISR 收缩与恢复全程可观测（4.2 节表） | **默认档**：异步复制，主从 offset 差就是"最终"的实测长度（演练二） |

两行读法：**竖着看**——同一家系统横跨全部档位，档位由"读路径 + 参数"当场决定，所以永远要反问"哪个操作、什么参数"；**横着看**——同一档位在三家的实现机理完全不同（多数派确认 / 单 leader 全序 / 单线程执行序），排查手段也因此不同：etcd 查 Raft 与 ReadIndex、Kafka 查 ISR 与 lag、Redis 查 offset 差。
...
| 系统 | 看哪个数 | 语义边界（容易踩的坑） |
|---|---|---|
| MySQL | `Seconds_Behind_Source` | 只衡量**重放**落后；IO 线程断开期间显示 NULL/0 但实际在掉队（`../13-middleware/mysql/02-backup-replication.md`） |
...
```bash
# [任意节点] 起一主一从（仿 ../13-middleware/redis/02-persistence-and-ha.md 的实验形态）
...
D：违反**因果一致**（读比已知因果更旧）；若业务需要，开启 causally consistent session + readConcern majority（mongodb 02 章 §4）。
</details>
