...
> 模块：19-distributed ｜ 建议时长：3.5 小时 ｜ 关联认证：—（无直接考点；Flink exactly-once 是 14 模块的理论深化，MySQL 的两阶段血缘在 13 模块）

## 学习目标

- 能指出 2PC 的两个结构性缺陷（协调者单点、参与者持锁阻塞）在故障时刻的具体表现，并解释 3PC 为什么没有任何生产系统采用
- 能给 TCC / Saga / 本地消息表各配一个业务例子，并说出各自的代价与适用边界
- 能拆穿"恰好一次"：讲清工程上的真相是 at-least-once + 幂等（映射 Flink checkpoint 两阶段提交与 Kafka 事务）
- 能按故障场景走完选型决策树，在一致性 × 吞吐 × 成本三个轴上给真实业务定方案，并说清 RabbitMQ confirm/outbox 与 Kafka 事务各自的边界
...
MySQL 内部就跑着一个 2PC：**redo log prepare → 写 binlog → commit**，以"binlog 是否完整落盘"作为崩溃恢复时提交或回滚的裁决依据。没有这层两阶段，就会出现"主库有、从库无"或反向的不一致——[13-middleware/mysql/01-innodb-fundamentals.md](../13-middleware/mysql/01-innodb-fundamentals.md) §4（三大日志与两阶段提交）有完整的反例推演。跨系统的 2PC 是同一思想的放大：**把"两个日志系统"换成"两个参与者系统"，把"本地 binlog"换成"协调者的决策日志"**。
...
**MySQL 半同步是它的"半个"版本**：主库 commit 后等至少一个从库 ACK binlog 才应答客户端（[13-middleware/mysql/02-backup-replication.md](../13-middleware/mysql/02-backup-replication.md) 半同步一节），超时（默认 10s）自动降级回异步——工程上承认"强一致窗口"有价格，价格太贵就显式降级并让监控看见。这正是 2PC 不肯做的事。
...
| 协调者决策日志 | **持久化的 checkpoint 本身**（状态目录落 PVC，见 [14-data-streaming/flink/02-deployment-and-exactly-once.md](../14-data-streaming/flink/02-deployment-and-exactly-once.md) §2） |
| 参与者 | 各算子，尤其是 sink |
| prepare | preCommit：barrier 对齐后 flush，事务句柄写进 checkpoint 状态 |
| commit | `notifyCheckpointComplete` 到达后 sink 真正 commit 事务 |

这个设计把经典 2PC 的两大缺陷各缓解了一半：协调者崩溃可从 checkpoint 恢复（决策日志持久化了）；但"commit 通知丢失"的窗口仍在——事务悬挂要靠 `transaction.timeout.ms` 兜底，所以才有那条硬约束：**transaction.timeout.ms 必须 > checkpoint 间隔 + 预期恢复时长，且 ≤ broker 的 transaction.max.timeout.ms**。三条前提与参数细节见 [flink 02 章 §5](../14-data-streaming/flink/02-deployment-and-exactly-once.md)（三前提缺一不可：内部 barrier 对齐、source 可重放、sink 事务性）。

### 5.2 Kafka：幂等生产者与事务

- **会话内幂等**：`enable.idempotence=true`（3.0 起默认）给每个 producer 发 PID，按 `<PID, partition>` 维护递增序列号，broker 端去重窗口拒绝重试重复。边界：跨会话（重启换 PID）、跨分区都不保证——见 [kafka 01 章 §7](../14-data-streaming/kafka/01-log-model-and-architecture.md) 的生产者幂等一节。
...
- **消费侧语义**：先处理后提交 = at-least-once，崩溃时重复消费，业务侧用幂等键（订单号、消息 UUID）兜底——[kafka 02 章 §8](../14-data-streaming/kafka/02-replication-and-reliability.md) 有"poll 到 500 条处理到 300 条被 OOMKill"的完整推演。
...
降级路径要记牢：若 sink 只能做幂等写入（按主键 upsert 的库），就退回 at-least-once + 幂等，吞吐与延迟都更好——这不是妥协，是正确选型（[flink 02 章 §5](../14-data-streaming/flink/02-deployment-and-exactly-once.md) 末尾原文）。
...
## 7. 故障驱动的选型决策树：一致性 × 吞吐 × 成本

前面六节按"协议谱系"讲。真实评审里的问题永远是具体的：**这个业务，崩的那一刻你愿意付出什么？** 选型不是挑框架，是先回答几个故障问题，答案会自动把方案筛出来：

```
[图] 分布式事务选型决策树（从故障出发，逐问收窄；三个轴在每片叶子上标价）

 Q1 跨系统的"同时成功/同时失败"，业务上真的需要吗？
 │  └─ 先试"不分布"：合并库 / 同库不同表 → 单机事务收工
 │     （最好的分布式事务是让事务不分布——这是所有方案里一致性和吞吐都最高的叶子）
 ▼ 确实跨系统
 Q2 协调者或参与者在事务中途崩溃/超时，允许对外可见的中间态吗？
 │
 ├─ 不允许任何中间态（对账必须逐分钱可解释：资金划拨、账务核心）
 │     ├─ 参与者少、事务毫秒级、流量可控 ──► XA/2PC（第 2 节）
 │     │        代价：参与者持锁等全局决定，高峰锁堆积是雪崩起点
 │     │        纪律：参与者尽量少、事务尽量短、锁等待必须有监控与超时回滚预案
 │     └─ 约束改不动 ──► 回去重新切分业务，把强一致边界缩小到一个库，
 │              而不是给烂边界套更强的协议
 │
 └─ 允许短暂中间态（最终一致可接受，靠补偿与对账兜住）
       │
       Q3 中间态被其他事务/用户看到会造成错误决策吗？（隔离性）
       ├─ 会：并发会踩到未决资源（库存被重复扣） ──► TCC
       │        Try 预留资源 = 应用层给中间态上锁；
       │        代价是一张表拆三个动作 + 空回滚/悬挂防御（第 4 节表）
       └─ 不会 / 流程长、人工在环 ──► Saga
                补偿逆序回滚；编排（中心协调器，定界容易）vs 协同（事件驱动，耦合低）
       │
       Q4 "一致"的落点，是不是"把一条消息可靠送到另一个系统"？（跨系统双写是多数场景的真身）
       ├─ 是 ──► 本地消息表 / outbox：业务与消息记录同一个本地事务（第 4 节表第三行）
       │        RabbitMQ 侧的组合拳见 7.1；若读与写都在 Kafka 内部，才有事务/epoch 可用（5.2 节）
       └─ 是流式管道（无限流，没有"单笔事务"概念） ──► at-least-once + 幂等（第 5/6 节）
```

叶子不是同质的，三根轴上各有定价：

| 方案 | 一致性 / 隔离档位 | 吞吐 | 成本记在哪 | 崩溃瞬间的行为 |
|---|---|---|---|---|
| 合并单库 | ACID 全档 | 最高 | 业务拆分与容量规划 | 单机故障半径（分布式的一切代价正是为绕开它而生） |
| XA/2PC | 强一致 + 参与者持锁隔离 | 低（锁跨 RTT） | 协调者运维 + 锁等待监控 | 参与者抱锁等决策，一个组件的故障放大成锁堆积 |
| TCC | 最终一致 + 应用层隔离（预留） | 中 | 每个接口三实现 + 幂等防御 | Confirm/Cancel 重试，中间态表现为资源被冻结 |
| Saga | 最终一致，无隔离 | 高 | 补偿逻辑 = 第二套业务代码 | 已走步骤逆序补偿，补偿本身必须幂等 |
| 本地消息表 | 最终一致 + 消息必发 | 高（本地事务多一行 INSERT） | relay 运维 + outbox 清理 + 下游幂等 | 消息延迟但不丢（积压可监控，自测 3） |
| at-least-once + 幂等 | 效果恰好一次 | 最高 | 幂等键/去重表设计 | 重放被幂等吸收，"重复"不可见 |

读树两条纪律：**问题全是故障句式**（"崩了怎么办"而不是"想要什么特性"）——一致性每升一档，同一行的吞吐与成本就在标价，没有免费的档位；**叶子是拼装件不是单选题**——核心链路"单库事务包住账务 + outbox 发通知 + 外围 Saga"是常见组合，决策树按数据流逐段走，不是整系统选一个方案。

### 7.1 落地到 RabbitMQ："消息事务"的正解不是 tx.select

本地消息表（第 4 节表第三行）在 RabbitMQ 世界怎么落地？先拆一个常见误会，再给组合拳：

- **AMQP 事务（`tx.select`/`tx.commit`）不是这里的答案**。它只是生产端信道上的同步提交点：只保证"本信道消息按序发出"，不覆盖路由成败、不等落盘、更不管消费端——对照第 2 节的 2PC，它连一个合格的"参与者"都算不上，同步阻塞还把吞吐打崩。官方文档明确用 publisher confirm 替代它，新代码不要再用（[13-middleware/rabbitmq/01-amqp-model.md](../13-middleware/rabbitmq/01-amqp-model.md) §3.1 的原话）。
- **正解是"本地消息表 + 三道闸 + 幂等消费"的组合**，每一件都对应已学机制：

| 事务里的角色 | RabbitMQ 机制 | 已学出处 |
|---|---|---|
| 业务成功 ⇔ 消息必发 | 业务库同事务写 orders + outbox，relay 扫表发布 | 本章第 4 节表第三行 |
| 消息真的进了队列 | `confirm.select` 异步回执；quorum 队列下 confirm 等的就是**多数派落盘**（第 03 章的 Raft 在队列层的化身） | rabbitmq 01 章 §3.1 / 02 章 §3 |
| 路由失败不静默丢 | `mandatory=true` + `basic.return` 退回处理 | rabbitmq 01 章 §3.1 |
| 消费侧"恰好一次" | 做不到：ack 前断线必重投（`redelivered=true`）→ 消费端幂等（第 6 节去重表/唯一键） | rabbitmq 01 章 §3.3 |

- **与 Kafka 事务的边界要答准**：Kafka 的 `transactional.id`/epoch 能把"读 Kafka → 处理 → 写回 Kafka"做成端到端 exactly-once（5.2 节），但下游一旦是外部系统就退回 at-least-once；RabbitMQ 连"跨队列原子组"都没有——confirm 是逐条回执，不是原子组。所以跨系统原子性必须回到 outbox：**消息系统能承包的是"不丢"，承包不了"恰好一次"；恰好一次永远由业务幂等收尾**（rabbitmq 01 章 §4 对这层边界的原版对照）。这也正是决策树 Q4 分叉的依据。
...
| Flink 作业频繁报事务超时 / 数据延迟可见 | `transaction.timeout.ms` ≤ checkpoint 间隔 | 调大事务超时或调小 checkpoint 间隔；同时不超过 broker 的 `transaction.max.timeout.ms`（[flink 02 章 §5](../14-data-streaming/flink/02-deployment-and-exactly-once.md) 三条硬约束） |
...
幂等的去重键是 `<PID, partition>` 上的序列号，而 PID 是 producer 启动时从 broker 领的**会话级**标识——重启后 PID 变了，broker 的去重窗口对新 PID 从零开始，崩溃前"已写入但未收到 ack"的那批消息重发后无法被识别为重复。跨会话要靠事务：固定的 `transactional.id` 让 coordinator 用递增 epoch 绑定"同一逻辑生产者"，新会话顶掉旧 epoch，旧 PID 的僵尸写入被拒绝——这就是 fencing（第 06 章）。见 [kafka 01 章 §7](../14-data-streaming/kafka/01-log-model-and-architecture.md)。
</details>
