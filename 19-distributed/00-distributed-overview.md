...
> 模块：19-distributed ｜ 建议时长：2 小时 ｜ 关联认证：—（无直接考点；本章是本模块 12 章的坐标系，把已学的 etcd/Kafka/Redis/ZooKeeper/Flink 经验正式"理论化"）

## 学习目标

- 能说清网络不可靠、时钟不同步、部分故障各自摧毁了单机世界的哪条隐含假设，并各举一个已学组件的对应机制
- 能解释"超时之后不知道对方做没做"这第三种状态为什么逼出幂等重试，并举出 kubectl / Kafka 里的现成例子
- 能画出本模块 12 章的学习地图，说清 etcd / Kafka / Redis / ZooKeeper / Flink 五处已学素材分别被哪一章收拢深化，以及 08~11 四章各自深化哪条主线
- 能纠正"CAP 三选二"的流行误读：分区是前提不是选项，取舍发生在分区期间，且同一系统里不同操作的取舍可以不同
- 能用"分区发生时谁停摆"一句话，给 etcd / ZK / Kafka(acks=all) / Redis 主从在 CAP 谱系上定位

前置约定：本模块全程不推导证明，只讲语义、代价、运维后果与面试答法。所有理论概念都先指回你在 04/10/13/14/18 模块里已经敲过的命令，交叉引用一律给相对路径。
...
- **已学证据**：`kubectl` 内部对 API 请求自带超时重试；Kafka 生产者 `acks=1` 超时重试同一批次可能写入两次，这是 `enable.idempotence` 存在的理由（`../14-data-streaming/kafka/02-replication-and-reliability.md`）。

运维后果：**一切"对下游有副作用的脚本"都必须按"可能被执行多次"来写**。你在 15-sre-methodology 学的 runbook 原则"先想回滚再敲回车"，在分布式语境里加一条"先想重放是否安全"。
...
- `Jenkins agent 时间漂移`会直接导致握手失败、agent 掉线（`../06-ci-cd/03-jenkins-and-github-actions.md` 常见坑表）；
- Flink 干脆放弃用墙钟定义时间，改用**事件自带的时间戳 + watermark**（`../14-data-streaming/flink/01-stream-processing-model.md` 第 2/3 节）——这是"不信任物理钟"最彻底的工程化。
...
```
ZK 3 节点（../18-bigdata/06-zookeeper.md 第 3 节）
挂 0 台:  读写正常
...
2. **放弃全员共识**：只要求多数派（quorum）同意，少数派的意见直接作废——这也是 3/5/7 奇数部署的根本原因（`../18-bigdata/06-zookeeper.md`："3 → 4 反而降低可用性"）；
...
```
理论地基本章+01+02            理论主体03~06                深化08~10                 落地
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────┐   ┌────────────────┐
│ 00 全景/CAP(本章)     │   │ 03 共识与复制:          │   │ 08 经典不可能性:     │   │ 07 排障方法论   │
│ 01 故障模型与时钟      │ ─►│    Raft/ZAB/ISR         │ ─►│    两将军/拜占庭/FLP │ ─►│ 11 协调服务三件套│
│ 02 一致性模型阶梯      │   │ 04 分布式事务:          │   │ 09 Paxos 深潜       │   │   etcd/Consul/  │
└──────────────────────┘   │    2PC/快照/幂等        │   │ 10 CRDT 与自动收敛   │   │   Nacos 选型    │
        ▲                  │ 05 分片与再平衡:        │   └─────────────────────┘   │ (lab 01~03      │
        │                  │    槽/哈希/范围         │                             │    贯穿全程)     │
        │                  │ 06 Gossip/成员关系/围栏  │                             └────────────────┘
        │ 全部概念先有实现样本再给名字
┌───────┴────────────────────────────────────────────────────────────────────┐
│ 已学素材: etcd(04章) Kafka(14模块) Redis(13模块) ZK(18模块) Flink(14模块)      │
└────────────────────────────────────────────────────────────────────────────┘
```

08~11 四章是 00~06 各主线的深化：08 给出全部工程妥协的理论出处（两将军/拜占庭/FLP/分布式快照），09 把 03 章的一句话共识史展开成 Paxos 协议本体，10 是 02 章最终一致与 03 章共识代价的合题（不靠共识的收敛），11 把 03 章 Raft 与 06 章 gossip 落到 etcd/Consul/Nacos 的工业选型；07 章的排障方法论覆盖全部 12 章。
...
| **Kafka** | ISR 动态收缩、HW/LEO、`min.insync.replicas`、KRaft、murmur2 分区（`../14-data-streaming/kafka/02-replication-and-reliability.md` 第 3 节） | 主备复制 / 顺序一致 / 分片路由 | 02 / 03 / 05 章 |
| **Redis** | 哨兵 quorum 与 majority 两阶段判定（SDOWN/ODOWN）、16384 槽、gossip cluster bus（`../13-middleware/redis/02-persistence-and-ha.md` 第 6 节起） | 故障检测 / 一致性哈希之辩 / gossip 成员关系 | 05 / 06 章 |
| **ZooKeeper** | ZAB 的 epoch>zxid>myid 选举、过半提交、脑裂防护、fencing token（`../18-bigdata/06-zookeeper.md` 第 3 节） | 共识 / 全序广播 / 租约与围栏 | 03 / 06 章 |
| **Flink** | barrier 对齐的分布式快照（Chandy-Lamport）、KafkaSink 两阶段提交（`../14-data-streaming/flink/02-deployment-and-exactly-once.md` 第 3/5 节） | 分布式快照 / 2PC / exactly-once 语义 | 04 章 |

三个"配角"也各有贡献：**MySQL 半同步**（`../13-middleware/mysql/02-backup-replication.md` 半同步复制一节）是"弱化版 2PC 单方面确认"；**MongoDB 副本集**（`../13-middleware/mongodb/02-replicaset-and-sharding.md` 第 4 节）给了 writeConcern/readConcern 这套"按请求选一致性"的最好样本；**双实例 Alertmanager**（`../10-pca/05-alerting-alertmanager.md` 6.2 节）是最终一致 + gossip 反熵的活标本。
