...
8. **跨模块相对链接**：三层目录（`模块/组件/章节`，如 13-middleware）中的章节文件，引用其他模块时相对链接必须以 `../../` 开头（例：`../../14-data-streaming/kafka/01-….md`）；同模块跨组件用 `../<component>/…`，组件内用 `./<file>` 或直接文件名。新增模块或调整目录层级后，全库 grep 校验：`grep -rn '](\.\./[0-9]' --include='*.md' <三层目录>` 应无命中。
